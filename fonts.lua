--[[
* XIDB Font Utilities
* Provides centralized font management for ImGui
]]--
...
-- Font scale presets for consistent sizing across UI
M.SCALES = {
    TINY = 0.8,
    SMALL = 0.9,
    NORMAL = 1.0,
...
-- Color presets for consistent theming
M.COLORS = {
...
-- Helper to set font scale safely
...
-- Helper to reset font scale to normal
...
function M.MeasureText(text, size_px, fallback_scale)
    local content = tostring(text or '')
    local font = M.GetFont(size_px)
    if font then
        local width = nil
        M.WithFont(size_px, function()
            width = imgui.CalcTextSize(content)
        end)
        if width ~= nil then
            return width
        end
    ...
    local scale = tonumber(fallback_scale) or ((tonumber(size_px) or DEFAULT_FONT_SIZE) / DEFAULT_FONT_SIZE)
    return imgui.CalcTextSize(content) * scale
end
...
function M.TextWrappedPx(text, color, size_px, fallback_scale)
    local content = tostring(text or '')
    local resolved_color = color or M.COLORS.WHITE
    local font = M.GetFont(size_px)
    if font then
        M.WithFont(size_px, function()
            imgui.PushStyleColor(ImGuiCol_Text, resolved_color)
    ...
    local scale = fallback_scale or ((tonumber(size_px) or DEFAULT_FONT_SIZE) / DEFAULT_FONT_SIZE)
    imgui.PushStyleColor(ImGuiCol_Text, resolved_color)
    if scale ~= M.SCALES.NORMAL then
        M.SetScale(scale)
    end
    imgui.TextWrapped(content)
    if scale ~= M.SCALES.NORMAL then
...
function M.TextWrapped(text, color, scale)
    local resolved_color = color or M.COLORS.WHITE
    imgui.PushStyleColor(ImGuiCol_Text, resolved_color)
...
-- Helper to render text with color and scale
...
-- Helper to render title text (large, gold, bold)
function M.Title(text)
    M.TextColoredPx(text, M.COLORS.GOLD, 24, M.SCALES.TITLE)
end

-- Helper to render massive title text (very large, gold, bold)
function M.Massive(text)
    M.TextColoredPx(text, M.COLORS.GOLD, 32, M.SCALES.MASSIVE)
end

-- Helper to render header text (medium, gold)
function M.Header(text)
    M.TextColoredPx(text, M.COLORS.GOLD, 18, M.SCALES.LARGE)
end

-- Helper to render recipe name (medium-large, light orange)
function M.RecipeName(text)
    M.TextColoredPx(text, M.COLORS.LIGHT_ORANGE, 18, M.SCALES.LARGE)
end

-- Helper to render ingredient text
function M.Ingredient(text)
    M.TextColoredPx(text, M.COLORS.LIGHT_GRAY, 18, M.SCALES.LARGE)
end

-- Helper to render label text (small, gray)
function M.Label(text)
    M.TextColoredPx(text, M.COLORS.GRAY, 18, M.SCALES.LARGE)
end
